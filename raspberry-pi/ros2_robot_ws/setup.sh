...
if command -v colcon &> /dev/null; then
  ...
  if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "To launch all nodes at once:"
    echo "  ./start_node.sh all"
    echo ""
    echo "Or launch via ROS2:"
    echo "  ros2 launch robot robot.launch.py"
    echo ""
    echo "Individual nodes (all auto-detect hardware):"
    echo "  ros2 run motor_control spi_controller_bridge   # motor controller on /dev/ttyUSB*"
    echo "  ros2 run imu_sensor imu_node                   # IMU + laser on /dev/ttyUSB*"
    echo "  ros2 run camera_sensor webcam_node             # USB webcam"
    echo "  ros2 run yolo_detection yolo_detection_node    # object detection"
    echo "  ros2 run encoder_odometry encoder_node         # wheel odometry"
    echo "  ros2 run robot_controller robot_controller     # high-level controller"
    echo "  ros2 run mqtt_bridge mqtt_bridge_node          # MQTT relay"
    echo ""
  else
    echo "✗ Build failed!"
    exit 1
